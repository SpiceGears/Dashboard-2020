...
var ui = {
	...
	varList: document.getElementById('Vars'),
	...
	elevator: {
		low: document.getElementById('lvl1'),
		middle: document.getElementById('lvl2'),
		high: document.getElementById('lvl3')
	},
...
function onValueChanged(key,value,isNew) {
	...
	switch(key) {
		...
		case '/SmartDashboard/ElevatorEncoder': //Trzeba dorobić w kodzie robota
			...
			if(value > 0.16 && value < 0.2) {
				ui.elevator.low.style.fill = 'green'
			}else {
				ui.elevator.low.style.fill = 'red'
			}
			if(value > 0.44 && value < 0.48) {
				ui.elevator.middle.style.fill = 'green'
			}else {
				ui.elevator.middle.style.fill = 'red'
			}
			if(value > 0.63 && value < 0.67) {
				ui.elevator.high.style.fill = 'green'
			}else {
				ui.elevator.high.style.fill = 'red'
			}
			break;
	...
	if (isNew && !document.getElementsByName(key)[0]) {
		...
		ui.varList.appendChild(div); // Add the div to the page
